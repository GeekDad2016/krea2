...
```bash
docker build -t your-dockerhub-user/krea-2-runpod:latest .
...
already uploaded. Set `HF_TOKEN` only if Hugging Face requests authentication,
and set `K2_CHECKPOINT_DIR` to override the default download directory. Turbo
is about 26.3 GB, so the attached RunPod volume needs sufficient free space.
